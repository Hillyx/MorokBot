...
def handle_start(message):
    ...
    user_markup.row('💼 Кошелек', '♻️ Обмен BTC')
    user_markup.row('🛡️ О сервисе', '📌 Акция')
    user_markup.row('💳 Партнерам')
    bot.send_message(message.from_user.id,
            '💰<b>CRYPTO ARBITR</b> - предлагает мгновенный обмен Dogecoin на Payeer по заниженной цене, а так же бесплатное хранилище Ваших BTC.', reply_markup=user_markup, parse_mode='HTML')

@bot.message_handler(content_types=['text'])
def handle_text(message):

    if message.text == "🛡️ О сервисе":
        bot.send_message(message.from_user.id, "<b>🛡️ О сервисе</b> \n  \nСервис для обмена Bitcoin. \nПополняй внутренний кошелек с помощью Qiwi или внешнего Bitcoin-кошелька.\n \n"
                         "Продавай эти BTC для вывода на Payeer. Или выводи на свой внешний Bitcoin-адрес.\n \n"
    ...
    if message.text == "📌 Акция":
        bot.send_message(message.from_user.id, "<b>📌 Акция</b>" "\n \n<b>CRYPTO ARBITR</b> проводит розыгрыш на <b>0.3 BTC</b>\n \n"
    ...
    def inline(c):
        ...
        if c.data == '📉 Продать':
            ...
            callback_button8 = types.InlineKeyboardButton(text="Payeer", callback_data="Перевод")
        ...
        if c.data == '📈 Купить':
            ...
            callback_button4 = types.InlineKeyboardButton(text="💵 Payeer", callback_data="💵 Payeer")
            ...
            message_id=c.message.message_id,
            text='<b>📈 Купить</b>\n \n'
                'Покупка BTC производится с помощью <b>Payeer</b> или переводом на многоразовый <b>Bitcoin-адрес</b> с внешнего кошелька.\n \n'
        ...
        if c.data == '💵 Payeer':
            ...
            bot.edit_message_text(
                ...
                'Курс обмена:\n'
                '<pre>1 BTC = ' + str(bitprice + 100) + ' RUB</pre>\n'
                '<pre>1 BTC = ' + str(bitpriceUSD + 8) + ' USD</pre>', parse_mode='HTML', reply_markup=keyboard7)
        ...
        if c.data == 'Перевод2':
            ...
            bot.send_message(c.message.chat.id, "<b>Номер:</b> P1010154499\n"
        ...
        if c.data == 'Оплатил':
            bot.edit_message_text(
                ...
                text= "✅ <b>Отлично</b>\n\n"
                      ...
                      "Если же вы допустили ошибку при переводе, то напишите в службу поддержки @Payforia_help. "
                      "Благодарим вас за выбор <b>CRYPTO ARBITR</b>.\n", parse_mode='HTML')
